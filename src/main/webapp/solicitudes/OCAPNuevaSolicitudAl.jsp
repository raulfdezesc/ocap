...
function construirCombos(){
	...
	cargarAnioConvocatoriaPGP('<bean:write name="OCAPNuevaSolicitudForm" property="CConvocatoriaId"/>');
...
function obtenerGradoModAnterior(){
   ...
   for (i=0; i < longListaGradosAnt; i++) {
      if (document.forms[0].CModAnteriorId.value == listaGradosAnterioresJS[i][0]) {
    	  var gradoAnterior = listaGradosAnterioresJS[i][1];
         document.forms[0].CGradoAnteriorId.value = gradoAnterior;
      }
   }
}
...
function irAltaSolicitud(){
   enviar('OCAPNuevaSolicitud.do?accion=irInsertar&desdeModificar=true');
}
...
function aceptarSolicitud(){
   if(validarFormulario(document.forms[0],'')) {
      ...
      //El grado que solicita no se corresponde con el siguiente al que tiene3
      var gradoAnteriorMasUno = (parseInt(document.forms[0].CGradoAnteriorId.value)+1);
      if ( isNaN(gradoAnteriorMasUno) ) {
    	  obtenerGradoModAnterior();
    	  gradoAnteriorMasUno = (parseInt(document.forms[0].CGradoAnteriorId.value)+1);
      }
      var gradoActual = parseInt(document.forms[0].CGrado_id.value);
      if( gradoAnteriorMasUno != gradoActual ) {
...
var listaConvocatoriasJS = new Array();
</script>

	<logic:present name="<%=Constantes.COMBO_CONVOCATORIAS%>">
		<logic:iterate id="convocatoria" name="<%=Constantes.COMBO_CONVOCATORIAS%>" type="OCAPConvocatoriasOT">
			<script type="text/javascript">
         linea = new Array();
         ...
         listaConvocatoriasJS[listaConvocatoriasJS.length] = linea;
      </script>
		</logic:iterate>
	</logic:present> 
   ...
   var listaConvocatoriasAltaJS = new Array();
	...
	<logic:present name="<%=Constantes.COMBO_CONVOCATORIAS_ALTA%>">
		<logic:iterate id="convocatoriaAlta" name="<%=Constantes.COMBO_CONVOCATORIAS_ALTA%>" type="OCAPConvocatoriasOT">
			<script type="text/javascript">
		 linea = new Array();
         linea[0] = '<bean:write name="convocatoriaAlta" property="CConvocatoriaId"/>';
         linea[1] = '<bean:write name="convocatoriaAlta" property="DAnioConvocatoria"/>';
...
function cargarAnioConvocatoriaPGP(idAnio){
		
	var anioConvocatoria ="";
	var longListaConvocatorias = listaConvocatoriasJS.length;
        
   for (i=0; i < longListaConvocatorias; i++) {
		    
      if (idAnio == listaConvocatoriasJS[i][0]) {    	    	         
    	  document.forms[0].anioConvocatoria.value = listaConvocatoriasJS[i][1];    	      	      	 
      }else if(idAnio == -1){
				...
				<fieldset>
					...
					<%if (jcylUsuario.getUser().getRol() != null && (jcylUsuario.getUser().getRol().equals(Constantes.OCAP_PUNTO_GESTION_PERIFE))){%>
						...
						<% 	if( session.getAttribute(Constantes.COMBO_CONVOCATORIAS)==null || 
								((ArrayList)session.getAttribute(Constantes.COMBO_CONVOCATORIAS)).size()==0)
							{ %>
								<script type="text/javascript">alert("No hay ninguna convocatoria disponible.");</script>
						<% 	} else { %>
								<!-- CONVOCATORIA -->
								<label class="labelReg" for="idVConvocatoria">
									Convocatoria: * <html:select property="CConvocatoriaId" styleId="CConvocatoriaId" onchange="javascript:cargarAnioConvocatoriaPGP(this.value);"
										styleClass="cbCuadros colocaConvocatoriaCBAl1" size="1">
					...
					<% 	} else { %>
						...
						<% 	} else { %>
								<!-- CONVOCATORIA -->
								<label class="labelReg" for="idVConvocatoria">
									Convocatoria: * <html:select property="CConvocatoriaId" onchange="javascript:cargarAnioConvocatoria(this.value);"
										styleClass="cbCuadros colocaConvocatoriaCBAl1" size="1">
										<html:option value="-1">Seleccione...</html:option>
					...
					<% 	} %>
						...
						<span class="mostrarCampoOculto" id="cajaEstatutarioFijo"
					...
					<fieldset>
						<div class="cuadroFieldsetLOPD">
							<p class="textoNormal margenDcho10">
								...
								<br />Doy mi CONSENTIMIENTO para que los datos e informaciones aportados en el  presente 
					...
					<div class="botonesPagina">
						<logic:notEqual name="opcion" value="<%=Constantes.GRS_FECHAS%>">
							<logic:notEqual name="opcion" value="<%=Constantes.GRS_EDC%>">
								<% 	
								if( Constantes.VER_DETALLE.equals(request.getParameter("opcion")) 
										&& ((String)session.getAttribute(Constantes.PARAM_SOLICITUD_MODIFICABLE)).equals("true") )
								{ %>
								<!-- div class="botonAccion">
									<span class="izqBoton"></span> <span class="cenBoton">
										<a href="javascript:irAltaSolicitud();"> <img
											src="imagenes/imagenes_ocap/edit.png"
											alt="Modificar Solicitud" /> <span> Modificar </span>
									</a>
									</span> <span class="derBoton"></span>
								</div-->
