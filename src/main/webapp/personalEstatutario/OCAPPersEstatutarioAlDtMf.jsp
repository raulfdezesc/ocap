...
function validar(tipoAccion){
   ...
   if  (validacion ==0){
      if (tipoAccion == '<%=Constantes.INSERTAR%>') {
         enviar('OCAPPersEstatutario.do?accion=insertar');    
      }else{
         enviar('OCAPPersEstatutario.do?accion=grabar');
      }
   }
}

function volverConsulta() {   
   enviar('OCAPPersEstatutario.do?accion=buscar&<%=Constantes.RECUPERAR_BUSQUEDA_ANTERIOR%>=<%=Constantes.SI%>');
}
...
<div class="ocultarCampo">
	...
	<div class="cuadroContenedor">
		<html:form action="/OCAPPersEstatutario.do">
			<logic:equal name="tipoAccion" value="<%=Constantes.INSERTAR%>">
				<div class="titulocajaformulario">Alta de Personal</div>
			</logic:equal>
			<logic:equal name="tipoAccion" value="<%=Constantes.VER_DETALLE%>">
				<div class="titulocajaformulario">Detalle de Personal</div>
			</logic:equal>
			<logic:equal name="tipoAccion" value="<%=Constantes.MODIFICAR%>">
				<div class="titulocajaformulario">Editar Personal</div>
			</logic:equal>
			<div class="cajaformulario">
				<fieldset class="normales">
					...
					<logic:notEqual name="tipoAccion" value="<%=Constantes.INSERTAR%>">
						<html:hidden property="CPersonal_id" />&nbsp;                  
</logic:notEqual>
					...
					<logic:notEqual name="tipoAccion"
						...
						<label class="obligado">* Nombre:</label>
						<html:text property="DNombre" tabindex="18"
							styleClass="inputColor colocaNombrePersEst" size="60"
							maxlength="100" />&nbsp;                  
</logic:notEqual>
