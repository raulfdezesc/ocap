...
function validar(tipoAccion){
   ...
   if  (validacion ==0){
      if (tipoAccion == '<%=Constantes.INSERTAR%>') {
         enviar('OCAPTipoGerencias.do?accion=insertar');    
      }else{
         enviar('OCAPTipoGerencias.do?accion=grabar');
      }
   }
}

function volverConsulta() {   
   enviar('OCAPTipoGerencias.do?accion=buscar&<%=Constantes.RECUPERAR_BUSQUEDA_ANTERIOR%>=<%=Constantes.SI%>');
}
...
<div class="ocultarCampo">
	...
	<div class="cuadroContenedor">
		<html:form action="/OCAPTipoGerencias.do">
			<logic:equal name="tipoAccion" value="<%=Constantes.INSERTAR%>">
				<div class="titulocajaformulario">Alta de Tipo de Gerencia</div>
			</logic:equal>
			<logic:equal name="tipoAccion" value="<%=Constantes.VER_DETALLE%>">
				<div class="titulocajaformulario">Detalle de Tipo de Gerencia</div>
			</logic:equal>
			<logic:equal name="tipoAccion" value="<%=Constantes.MODIFICAR%>">
				<div class="titulocajaformulario">Editar Tipo de Gerencia</div>
			</logic:equal>
			<div class="cajaformulario">
				<fieldset class="normales">
					...
					<logic:notEqual name="tipoAccion"
						...
						<label class="obligado">* Nombre:</label>
						<html:text property="DNombre" tabindex="18"
							styleClass="inputColor colocaNombreTipoG" size="60"
							maxlength="100" />&nbsp;                  
</logic:notEqual>
					...
					<logic:equal name="tipoAccion" value="<%=Constantes.VER_DETALLE%>">
						<html:text property="DDescripcion" tabindex="18"
							styleClass="inputColor colocaDescTipoG" size="60" maxlength="200"
							readonly="true" />&nbsp;                  
</logic:equal>
					<logic:notEqual name="tipoAccion"
						value="<%=Constantes.VER_DETALLE%>">
