...
function validar(tipoAccion){
   ...
   if  (validacion ==0){
      if (tipoAccion == '<%=Constantes.INSERTAR%>') {
         enviar('OCAPTiposFirmante.do?accion=insertar');    
      }else{
         enviar('OCAPTiposFirmante.do?accion=grabar');
      }
   }
}

function volverConsulta() {   
   enviar('OCAPTiposFirmante.do?accion=buscar&<%=Constantes.RECUPERAR_BUSQUEDA_ANTERIOR%>=<%=Constantes.SI%>');
}
...
<div class="ocultarCampo">
	...
	<div class="cuadroContenedor">
		<html:form action="/OCAPTiposFirmante.do">
			<logic:equal name="tipoAccion" value="<%=Constantes.INSERTAR%>">
				<div class="titulocajaformulario">Alta de Tipo de Firmante</div>
			</logic:equal>
			<logic:equal name="tipoAccion" value="<%=Constantes.VER_DETALLE%>">
				<div class="titulocajaformulario">Detalle de Tipo de Firmante</div>
			</logic:equal>
			<logic:equal name="tipoAccion" value="<%=Constantes.MODIFICAR%>">
				<div class="titulocajaformulario">Editar Tipo de Firmante</div>
			</logic:equal>
			<div class="cajaformulario">
				<fieldset class="normales">
					...
					<logic:notEqual name="tipoAccion"
						...
						<label class="obligado">* Nombre:</label>
						<html:text property="DNombre" tabindex="18"
							styleClass="inputColor colocaNomTipoFirm" size="60"
							maxlength="100" />&nbsp;                  
</logic:notEqual>
